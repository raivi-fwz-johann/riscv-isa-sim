...
bool __attribute__((weak)) svnapot_vsatp(){
    TEST_START();
    ...
    L0_pagetable[0]  = PTE_N | PTE_V | PTE_RWX | PTE_AD | page_addr_lower >> 2 | 0b1000 << 10;
    L0_pagetable[16] = PTE_N | PTE_V | PTE_RWX | PTE_AD | page_addr_lower >> 2 | 0b1000 << 10;
    ...
    *( (reg_t*)page_addr_upper + 8 ) = read_data_upper;
    ...
    // Spike does not has TLB, so exception should be triggered
    read_data_temp = *( (reg_t*)napot_test_addr );
    TEST_COMPARE("Check that read from NAPOT test address trigger exception", true, excpt.triggered);
