...
extern reg_t sv39_hgatp_return_addr;
...
static void exception_shandler(){
    ...
    CSRW(sepc, (reg_t)(&sv39_hgatp_return_addr));
}
...
bool __attribute__((weak)) sv39_x_hgatp(){
    TEST_START();
...
static void exception_execute() {
    asm volatile (
        ...
        ".globl sv39_hgatp_return_addr\n\t"
        "sv39_hgatp_return_addr: \n\t"
        "nop \n\t"
    );
}
