...
bool __attribute__((weak)) mstatus_mprv(){
    ...
    // set MPV to zero
    csr_set_field(mstatus, MSTATUS_MPV_OFF, MSTATUS_MPV_LEN, 1UL);
    mpv = csr_get_field(mstatus, MSTATUS_MPV_OFF, MSTATUS_MPV_LEN);
    TEST_COMPARE("check mstatus.mpv is 1", 1, mpv);
    
    // set vsatp register for VS mode address translation
    satp_value = get_test_superpage_ppn();
    satp_value |= SATP_SV39<< SATP_MODE_OFF;
    CSRW(vsatp, satp_value);
    
    // check that when MPRV is 0 and MPP is MODE_S and MPV is 1, translation is not used  
    ...
    // check that when MPRV is 1 and MPP is MODE_M and MPV is 1, translation is not used  
    ...
    // check that when MPRV is 1 and MPP is MODE_S and MPV is 1, translation is used
    ...
    csr_set_field(mstatus, MSTATUS_MPP_OFF, MSTATUS_MPP_LEN, MODE_M);
    csr_set_field(mstatus, MSTATUS_MPRV_OFF, MSTATUS_MPRV_LEN, 0);
    csr_set_field(mstatus, MSTATUS_MPV_OFF, MSTATUS_MPV_LEN, 0UL);
