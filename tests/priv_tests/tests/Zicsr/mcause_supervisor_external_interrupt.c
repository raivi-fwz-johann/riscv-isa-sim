...
extern reg_t mcause_return_address_seip;
...
static void mcause_mhandler(){
    ...
    CSRW(mepc, (reg_t)&mcause_return_address_seip);
}

bool __attribute__((weak)) mcause_supervisor_external_interrupt(){
    TEST_START();
    ...
    asm volatile (
        "ecall \n"
        ".globl mcause_return_address_seip \n"
        "mcause_return_address_seip: \n"
        "nop \n"
    );
