...
bool __attribute__((weak)) medeleg_store_page_fault(){
    TEST_START();
    ...
    CSRW(medeleg, 1 << CAUSE_STORE_PAGE_FAULT);
    medeleg_reg = CSRR(medeleg);
    TEST_COMPARE("clear and check that medeleg bit is 1. Delegation is ON", 1 << CAUSE_STORE_PAGE_FAULT, medeleg_reg);
    ...
    TEST_COMPARE("check trap cause",CAUSE_STORE_PAGE_FAULT , medeleg_cause);
