...
extern reg_t mcause_return_address_stip;
...
static void mcause_mhandler(){
    ...
    CSRW(mepc, (reg_t)&mcause_return_address_stip);
}

bool __attribute__((weak)) mcause_supervisor_timer_interrupt(){
    TEST_START();
    ...
    CSRW(mip, 1 << MIP_STIP_OFF);// to deliver timer interrupt to S-mode

    // csr_set_field(mip, MIP_STIP_OFF, MIP_STIP_LEN, 1); // to deliver timer interrupt to S-mode
    // temp = csr_get_field(mip, MIP_STIP_OFF, MIP_STIP_LEN);
    // TEST_COMPARE("check mip.stip is set", 1, temp);
    ...
    asm volatile (
        "ecall \n"
        ".globl mcause_return_address_stip \n"
        "mcause_return_address_stip: \n"
        "nop \n"
    );
