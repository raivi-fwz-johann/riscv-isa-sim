...
bool __attribute__((weak)) medeleg_illigal_instruction(){
    TEST_START();
    ...
    medeleg_reg = CSRR(medeleg);
    TEST_COMPARE("clear and check that medeleg bit is 1. Delegation is ON", 1 << CAUSE_ILLEGAL_INSTRUCTION, medeleg_reg);
    ...
    TEST_COMPARE("check trap cause", CAUSE_ILLEGAL_INSTRUCTION, medeleg_cause);
