...
bool __attribute__((weak)) medeleg_load_address_misaligned(){
    TEST_START();
    ...
    medeleg_reg = CSRR(medeleg);
    TEST_COMPARE("clear and check that medeleg bit is 1. Delegation is ON", 1 << CAUSE_MISALIGNED_LOAD, medeleg_reg);
    ...
    TEST_COMPARE("check trap cause", CAUSE_MISALIGNED_LOAD, medeleg_cause);
