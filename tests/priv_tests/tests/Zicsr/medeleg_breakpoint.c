...
bool __attribute__((weak)) medeleg_breakpoint(){
    TEST_START();
    ...
    medeleg_reg = CSRR(medeleg);
    TEST_COMPARE("clear and check that medeleg bit is 1. Delegation is ON", 1 << CAUSE_BREAKPOINT, medeleg_reg);
    ...
    TEST_COMPARE("check trap cause", CAUSE_BREAKPOINT, medeleg_cause);
