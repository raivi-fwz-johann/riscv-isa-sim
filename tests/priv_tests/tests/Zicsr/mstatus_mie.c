...
void __attribute__((weak)) mstatus_mie_handler(){
    excpt.triggered = true;
    csr_set_field(mie, MIE_MTIE_OFF, MIE_MTIE_OFF, 0);
}

bool __attribute__((weak)) mstatus_mie(){
    TEST_START();

    reg_t mtie, mtip, mie;

    set_mhandler(mstatus_mie_handler);                                                                                                                                                                             
