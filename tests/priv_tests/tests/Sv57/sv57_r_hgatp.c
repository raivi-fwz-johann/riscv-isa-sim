...
bool __attribute__((weak)) sv57_r_hgatp(){
    TEST_START();
    ...
    // check that page with R flag equal 0 cause page fault exception
    ...
    TEST_COMPARE("check that interrupt is triggered when R is 0", true, excpt.triggered);
    TEST_COMPARE("check trap cause CAUSE_LOAD_GUEST_PAGE_FAULT", CAUSE_LOAD_GUEST_PAGE_FAULT, exception_cause);


    // check that page with R flag equal 1 not cause page fault exception
    excpt.triggered = false;
    excpt.for_testing = true;
    ...
    TEST_COMPARE("check that interrupt is not triggered when R is 1", false, excpt.triggered);
