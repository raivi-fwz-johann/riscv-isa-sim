...
bool __attribute__((weak)) sv57_u_hgatp(){
    TEST_START();
    ...
    // check that page with U flag equal 0 in hgatp cause page fault exception in VS mode
    ...
    TEST_COMPARE("check that interrupt is triggered when U is 0", true, excpt.triggered);
    TEST_COMPARE("check trap cause CAUSE_LOAD_GUEST_PAGE_FAULT", CAUSE_LOAD_GUEST_PAGE_FAULT, exception_cause);


    // check that page with U flag equal 1 in hgatp not cause page fault exception in VS mode
    excpt.triggered = false;
    excpt.for_testing = true;
    ...
    TEST_COMPARE("check that interrupt is not triggered when U is 1", false, excpt.triggered);
