...
bool __attribute__((weak)) shvstvecd(){
    TEST_START();

    const reg_t mode_direct = 0;
    const reg_t mode_vectored = 1;

    reg_t mode, base, old_vstvec;
    ...
    csr_set_field(vstvec, VSTVEC_BASE_OFF, VSTVEC_BASE_LEN, -1L);
    base = csr_get_field(vstvec, VSTVEC_BASE_OFF, VSTVEC_BASE_LEN);
    TEST_COMPARE("set base to all ones and check that all bits are writable", (reg_t)(-1L) >> 2, base);

    // restore old stvec value
    CSRW(vstvec, old_vstvec);
