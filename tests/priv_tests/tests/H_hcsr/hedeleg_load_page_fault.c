...
bool __attribute__((weak)) hedeleg_load_page_fault(){
    TEST_START();
    ...
    hedeleg_reg = CSRR(hedeleg);
    TEST_COMPARE("clear and check that hedeleg bit is 1. Delegation is ON", 1 << CAUSE_LOAD_PAGE_FAULT, hedeleg_reg);
    ...
    TEST_COMPARE("check trap cause", CAUSE_LOAD_PAGE_FAULT, hedeleg_cause);
