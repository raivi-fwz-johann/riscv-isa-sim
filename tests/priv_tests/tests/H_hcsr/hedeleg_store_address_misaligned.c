...
bool __attribute__((weak)) hedeleg_store_address_misaligned(){
    TEST_START();
    ...
    CSRW(hedeleg, 1 << CAUSE_MISALIGNED_STORE);
    hedeleg_reg = CSRR(hedeleg);
    TEST_COMPARE("clear and check that hedeleg bit is 1. Delegation is ON", 1 << CAUSE_MISALIGNED_STORE, hedeleg_reg);
    ...
    TEST_COMPARE("check trap cause", CAUSE_MISALIGNED_STORE, hedeleg_cause);
