...
bool __attribute__((weak)) hedeleg_breakpoint(){
    TEST_START();
    ...
    hedeleg_reg = CSRR(hedeleg);
    TEST_COMPARE("clear and check that hedeleg bit is 1. Delegation is ON", 1 << CAUSE_BREAKPOINT, hedeleg_reg);
    ...
    TEST_COMPARE("check trap cause", CAUSE_BREAKPOINT, hedeleg_cause);
