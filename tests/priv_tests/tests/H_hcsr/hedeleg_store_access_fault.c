...
bool __attribute__((weak)) hedeleg_store_access_fault(){
    TEST_START();
    ...
    CSRW(hedeleg, 1 << CAUSE_STORE_ACCESS);
    hedeleg_reg = CSRR(hedeleg);
    TEST_COMPARE("clear and check that hedeleg bit is 1. Delegation is ON", 1 << CAUSE_STORE_ACCESS, hedeleg_reg);
    ...
    TEST_COMPARE("check trap cause", CAUSE_STORE_ACCESS, hedeleg_cause);
