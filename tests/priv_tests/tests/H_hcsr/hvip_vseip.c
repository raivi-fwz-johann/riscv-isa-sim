...
void __attribute__((weak)) hvip_seip_handler(){
    excpt.triggered = true;
    csr_set_field(sstatus, SSTATUS_SPIE_OFF, SSTATUS_SPIE_LEN, 0);
}

bool __attribute__((weak)) hvip_vseip(){
    TEST_START();

    reg_t seie, hvip, sie;

    set_vshandler(hvip_seip_handler);                                                                                                                                                                             
    ...
    // check that interrupt is not triggered when hvip.vseip is clear
    ...
    TEST_COMPARE("check hvip.vseip is clear", 0, hvip);
