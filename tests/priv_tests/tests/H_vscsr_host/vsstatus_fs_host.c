...
bool __attribute__((weak)) vsstatus_fs_host(){
    TEST_START();
    ...
    set_shandler(vsstatus_fs_handler);                                                                                                                                                                             
    ...
    csr_set_field(vsstatus, VSSTATUS_FS_OFF, VSSTATUS_FS_LEN, fs_off);
    fs = csr_get_field(vsstatus, VSSTATUS_FS_OFF, VSSTATUS_FS_LEN);
    TEST_COMPARE("set and check that vsstatus.fs is in OFF state", fs_off, fs);
    ...
    csr_set_field(vsstatus, VSSTATUS_FS_OFF, VSSTATUS_FS_LEN, fs_initial);
    fs = csr_get_field(vsstatus, VSSTATUS_FS_OFF, VSSTATUS_FS_LEN);
    TEST_COMPARE("set and check that vsstatus.fs is in INITIAL state", fs_initial, fs);
    ...
    fs = csr_get_field(vsstatus, VSSTATUS_FS_OFF, VSSTATUS_FS_LEN);
    TEST_COMPARE("check that vsstatus.fs is in DIRTY state", fs_dirty, fs);

    fs = csr_get_field(sstatus, SSTATUS_FS_OFF, SSTATUS_FS_LEN);
    TEST_COMPARE("check that vsstatus.fs is in DIRTY state", fs_dirty, fs);
    
    // clear state and perform FP operation again
    csr_set_field(vsstatus, VSSTATUS_FS_OFF, VSSTATUS_FS_LEN, fs_clean);

    fs = csr_get_field(vsstatus, VSSTATUS_FS_OFF, VSSTATUS_FS_LEN);
    TEST_COMPARE("set and check that vsstatus.fs is in CLEAN state", fs_clean, fs);

    fs = csr_get_field(sstatus, SSTATUS_FS_OFF, SSTATUS_FS_LEN);
    TEST_COMPARE("set and check that sstatus.fs is in DIRTY state", fs_dirty, fs);
