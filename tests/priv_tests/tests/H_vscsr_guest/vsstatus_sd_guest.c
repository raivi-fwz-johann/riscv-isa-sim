...
bool __attribute__((weak)) vsstatus_sd_guest(){
    TEST_START();
    ...
    sd = csr_get_field(sstatus, SSTATUS_SD_OFF, SSTATUS_SD_LEN);
    TEST_COMPARE("check that sstatus.sd is clear", 0, sd);
    ...
    fs = csr_get_field(sstatus, SSTATUS_FS_OFF, SSTATUS_FS_LEN);
    TEST_COMPARE("check that sstatus.fs is in DIRTY state", state_dirty, fs);

    sd = csr_get_field(sstatus, SSTATUS_SD_OFF, SSTATUS_SD_LEN);
    TEST_COMPARE("check that sstatus.sd is set", 1, sd);
    ...
    vs = csr_get_field(sstatus, SSTATUS_VS_OFF, SSTATUS_VS_LEN);
    TEST_COMPARE("check that sstatus.vs is in DIRTY state", state_dirty, vs);
