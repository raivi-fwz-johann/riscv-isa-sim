...
constexpr size_t LEGACY_PK_MAX_FILES = 128;

void ensure_fd_slot(std::vector<int>& fds, reg_t guest_fd)
{
  if (guest_fd >= fds.size())
    fds.resize(guest_fd + 1, -1);
}

void assign_fd(std::vector<int>& fds, reg_t guest_fd, int host_fd)
{
  ensure_fd_slot(fds, guest_fd);
  if (fds[guest_fd] >= 0)
    close(fds[guest_fd]);
  fds[guest_fd] = host_fd;
}
...
std::string get_fd_path(int fd)
{
  ...
  const ssize_t len = readlink(proc_path, path.data(), path.size() - 1);
  if (len < 0)
    return {};
...
{
  static const std::string deleted_suffix = " (deleted)";
...
void htif_t::save_checkpoint_host_state(std::ostream& out) const
{
  ...
  const auto& fds = syscall_proxy.get_fds().raw_fds();
  for (reg_t guest_fd = 0; guest_fd < fds.size(); ++guest_fd) {
    const int host_fd = fds[guest_fd];
    if (host_fd < 0)
      continue;

    if (guest_fd == 0) {
      out << "fd " << guest_fd << " stdio 0" << std::endl;
      continue;
    }

    if (guest_fd == 1 || guest_fd == 2) {
      out << "fd " << guest_fd << " stdio 1" << std::endl;
      continue;
    }
    ...
    errno = 0;
    const off_t offset = lseek(host_fd, 0, SEEK_CUR);
    const long long saved_offset = offset >= 0 ? static_cast<long long>(offset) : -1;
...
{
  std::vector<int> restored_fds(3, -1);
  std::string line;
  ...
  try {
    while (std::getline(in, line)) {
      ...
      if (tag == "cwd") {
        ...
        if (!(iss >> encoded_cwd))
          throw std::runtime_error("malformed checkpoint cwd record");
      ...
      if (!(iss >> guest_fd >> type))
        throw std::runtime_error("malformed checkpoint fd record");

      if (type == "stdio") {
        int stdio_fd;
        if (!(iss >> stdio_fd))
          throw std::runtime_error("malformed checkpoint stdio record");

        const int host_fd = dup(stdio_fd);
        if (host_fd < 0)
          throw std::runtime_error("failed to dup stdio while restoring checkpoint state");

        assign_fd(restored_fds, guest_fd, host_fd);
        continue;
      }
      ...
      if (type == "path") {
        ...
        if (!(iss >> flags >> saved_offset >> encoded_path))
          throw std::runtime_error("malformed checkpoint path record");

        const std::string path = hex_decode(encoded_path);
        const int host_fd = open(path.c_str(), flags);
        if (host_fd < 0)
          throw std::runtime_error("failed to reopen checkpoint fd path: " + path);

        if (saved_offset >= 0 &&
            lseek(host_fd, static_cast<off_t>(saved_offset), SEEK_SET) < 0) {
          close(host_fd);
      ...
      if (type == "opaque") {
        ensure_fd_slot(restored_fds, guest_fd);
    ...
    for (int fd : restored_fds)
      if (fd >= 0)
        close(fd);
    throw;
  }

  syscall_proxy.get_fds().replace_all(std::move(restored_fds));
}

bool htif_t::restore_legacy_checkpoint_host_state()
{
  if (targs.size() < 2 || targs[0] == "none")
    return false;

  class nop_memif_t : public memif_t {
   public:
    explicit nop_memif_t(htif_t* htif) : memif_t(htif) {}
  ...
  reg_t dummy_entry = 0;
  try {
    auto symbols = load_elf(targs[0].c_str(), &nop_memif, &dummy_entry, 0,
                            expected_xlen);
  ...
  const int host_fd = open(payload_path.c_str(), O_RDONLY);
  if (host_fd < 0) {
