...
if (vstart_eg < vl_eg) {
  const EGU8x16_t round_key = P.VU.elt_group<EGU8x16_t>(rs1_num, 0);
  for (reg_t idx_eg = vstart_eg; idx_eg < vl_eg; ++idx_eg) {
    EGU8x16_t &new_state = P.VU.elt_group<EGU8x16_t>(rd_num, idx_eg, true);
    EGU8x16_t state = P.VU.elt_group<EGU8x16_t>(rs2_num, idx_eg);
    //GOST_XS(state, round_key);
    new_state = xvkgost_kuznz_xor(state, round_key);
  }
}
