...
if (vstart_eg < vl_eg) {
  const EGU8x16_t scalar_key = P.VU.elt_group<EGU8x16_t>(vs1_num, 0);
  for (reg_t idx_eg = vstart_eg; idx_eg < vl_eg; ++idx_eg) {
    EGU8x16_t &new_state = P.VU.elt_group<EGU8x16_t>(vd_num, idx_eg, true);
    EGU8x16_t state = P.VU.elt_group<EGU8x16_t>(vs2_num, idx_eg);
    GOST_XS(state, scalar_key);
    new_state = xvcgost_kuzL_enc(state);
  }
}
P.VU.vstart->write(0);
