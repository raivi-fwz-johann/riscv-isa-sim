...
auto xvcgost_kuzn = [&](const uint8_t R16_mat[16][16], const EGU8x16_t &state) {
  EGU8x16_t new_state;
...
auto xvcgost_kuzL_enc = [&](const EGU8x16_t &state) {
  ...
  return xvcgost_kuzn(R16_Enc, state);
};

auto xvcgost_kuzL_dec = [&](const EGU8x16_t &state) {
  ...
  return xvcgost_kuzn(R16_Dec, state);
};

auto xvcgost_sbg_P = [](const EGU64x4_t &hi, const EGU64x4_t &lo, bool high) {
  static const uint8_t Tau_hi[32] {
     0,  8, 16, 24, 32, 40, 48, 56,  1,  9, 17, 25, 33, 41, 49, 57,
     2, 10, 18, 26, 34, 42, 50, 58,  3, 11, 19, 27, 35, 43, 51, 59,
  };
  static const uint8_t Tau_lo[32] {
  ...
  uint8_t *hi_bytes = (uint8_t*)hi.data();
  uint8_t *lo_bytes = (uint8_t*)lo.data();
  for (unsigned i = 0; i < 32; i++) {
    uint8_t val = Tau[i];
    r[i] = (val > 31) ? lo_bytes[val - 32] : hi_bytes[val];
  }
  return res;
};

auto xvcgost_sbg_L = [](const EGU64x4_t &state) {
