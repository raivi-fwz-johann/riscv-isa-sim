...
void checkpoint_t::save() {
  ...
  save_htif_file(config.snapshot_save_name);
}
...
void checkpoint_t::cpu_deserialize(const char *load_name) {
  ...
  // Restore HTIF tohost/fromhost addresses from .htif file (if present)
  std::string htif_load_name(load_name);
  htif_load_name.append(".htif");
  std::ifstream htif_fin(htif_load_name);
  if (htif_fin.good()) {
    ...
    htif_fin.close();
    std::cerr << "NOTE: restored HTIF addresses from " << htif_load_name
              << ": tohost=0x" << std::hex << sim_ptr->get_tohost_addr()
              << " fromhost=0x" << sim_ptr->get_fromhost_addr() << std::endl;
  }
}
...
void checkpoint_t::save_htif_file(const char *save_name) {
  std::string htif_name(save_name);
  htif_name.append(".htif");

  auto *sim_ptr = static_cast<sim_t*>(sim);

  std::ofstream fout(htif_name);
  if (!fout.is_open()) {
    std::cerr << "warning: cannot create " << htif_name << std::endl;
    return;
  }

  fout << "tohost_addr:0x" << std::hex << sim_ptr->get_tohost_addr() << std::endl;
  fout << "fromhost_addr:0x" << std::hex << sim_ptr->get_fromhost_addr() << std::endl;
  fout.close();

  std::cerr << "NOTE: saved HTIF state to " << htif_name << std::endl;
}
