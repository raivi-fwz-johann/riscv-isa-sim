...
size_t run_helper_t::_step(sim_t *spike_sim, size_t n, size_t proc) {
  ...
  if (spike_sim->tohost_addr == 0) {
    while (!spike_sim->should_exit() && n > 0) {
  ...
  if (spike_sim->get_host_disabled()) {
    while (!spike_sim->should_exit() && n > 0) {
      auto steps_done = spike_sim->idle_ext(n, proc);
      if (steps_done == 0) {
        break;
  ...
  while (!spike_sim->should_exit() && n > 0)
  {
    ...
    if (spike_sim->not_in_step()) {
      try {
        if ((tohost = spike_sim->from_target(spike_sim->mem.read_uint64(spike_sim->tohost_addr))) != 0)
          spike_sim->mem.write_uint64(spike_sim->tohost_addr, target_endian<uint64_t>::zero);
      } catch (mem_trap_t& t) {
    ...
    try {
      if (tohost != 0 and spike_sim->not_in_step()) {
      ...
      if (spike_sim->not_in_step()) spike_sim->device_list.tick();
    } catch (mem_trap_t& t) {
    ...
    if (spike_sim->not_in_step()) {
      try {
        if (!fromhost_queue.empty() && !spike_sim->mem.read_uint64(spike_sim->fromhost_addr)) {
          spike_sim->mem.write_uint64(spike_sim->fromhost_addr, spike_sim->to_target(fromhost_queue.front()));
          fromhost_queue.pop();
        }
      } catch (mem_trap_t& t) {
