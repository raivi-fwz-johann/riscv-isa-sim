...
static int spike_init(int argc, char** argv, sim_t *&spike_sim, cfg_t &cfg, std::function<void(sim_t*)> callback = [](sim_t*){})
{
  //// RiVAI: gprof add --ZQ
  bool prof_en = false;
  ...
  if (sm_config.is_simpoint_enabled()) {
    spike_sim->set_simpoint_module(sm_config);
  }
  //// RiVAI: simpoint add end --YC
  if (use_rbb) {
  ...
  spike_sim->configure_log(log, log_commits, log_commits_stant);
